...
public class ConsoleDialog {
    ...
    private Purse purse;
    // we only use one currency in this dialog. Save it so user doesn't have to
    // re-enter it.
    ...
    public void run() {
        while (currency.isBlank())
            currency = getReply("What currency to use? ");
    ...
    public void withdrawDialog() {
        ...
        if (scanline.hasNextDouble()) {
            ...
            Valuable[] money = purse.withdraw(amount, currency);
            if (money == null)
